...
const legalQualification = bounded(String.raw`iddia\s+edildi|iddiası|öne\s+sürüldü|savcılığa\s+göre|mahkemeye\s+göre|mahkeme\s+kararıyla|kesinleşmiş\s+(?:karar|hüküm)|hüküm\s+giydi|şüpheli|sanık|tutuklandı|gözaltına\s+alındı|hakkında\s+soruşturma|suçlamasıyla|suçlaması|gazetesinin\s+haberine\s+göre|kaynağının\s+haberine\s+göre`);
